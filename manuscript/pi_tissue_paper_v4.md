...
- **GTEx v8** (human; bulk RNA-seq; 263 donors aged 20–79 with matched samples in 6 tissues; additional 15-tissue panel for per-tissue noise rates). Files: `GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_tpm.gct.gz` and the v8 sample/subject annotation files, accessed via dbGaP phs000424.v8 in January 2026. Source: Mele et al. 2015; GTEx Consortium 2020. Used for the primary variance decomposition (π_tissue, π_donor, π_residual), per-tissue noise-rate analysis, and sex stratification (Sections 3.1–3.3, 3.5, 3.6).

- **Tabula Muris Senis — FACS** (mouse; Smart-seq2 scRNA-seq; 110,824 cells across 23 tissues; ages 3 and 24 months). Version: `tabula-muris-senis-facs-processed-official-annotations.h5ad` from figshare doi:10.6084/m9.figshare.12654728 (2020 release), accessed February 2026. Source: Schaum et al. 2020 Nature 583:596. Used for single-cell validation of compositional versus cell-intrinsic origin of the bulk residual growth (Section 3.4).

- **Tabula Muris Senis — Droplet** (mouse; 10x Chromium scRNA-seq; 245,389 cells across 16 tissues; ages 1, 3, 18, 21, 24, 30 months). Version: `tabula-muris-senis-droplet-processed-official-annotations.h5ad`, same figshare repository (doi:10.6084/m9.figshare.12654728), accessed February 2026. Source: Schaum et al. 2020. Used as the independent-platform single-cell validation with lower age-related gene-detection bias (Section 3.4).

- **Calico rat aging atlas** (rat; 10x Chromium scRNA-seq; 218,971 cells across 9 tissues; three conditions: young 5 mo, old ad libitum 27 mo, old caloric restriction 27 mo; n = 2 animals per condition). Source: Zou et al. 2022 bioRxiv 2022.11.28.518204. Data accessed via GEO GSE141784 in March 2026; pre-processed h5ad from the accompanying companion-paper release (see code repository). Used for the CR intervention test on π_tissue (Section 3.7).

- **Macaque cross-species atlas** (rhesus macaque; bulk RNA-seq; 500 samples from 17 animals across 32 tissues; ages 3–27 years). Source: Li & Kong 2025, figshare 26963386 (v1, released 2024), accessed April 2026. Used for cross-species scaling of π erosion rate alongside mouse, rat, and human (Section 3.8).
...
**Synthesis.** The bulk residual growth in GTEx (≈ +0.026) is consistent with both compositional drift and a modest cell-intrinsic component. We can estimate their relative contributions from the τ-stratified decomposition (Fig. 2C). Tissue-specific genes (τ > 0.8, n = 395), which by definition are expressed in only one or two tissues and are therefore insensitive to shifts in immune or stromal cell-type proportions, show Δπ_tissue = −0.011. Ubiquitous genes (τ < 0.3, n = 7,550), which are expressed in every cell type and therefore fully susceptible to composition shifts, show Δπ_tissue = −0.041. If we take the tissue-specific decline (−0.011) as a floor estimate of the cell-intrinsic contribution and the ubiquitous decline (−0.041) as a ceiling estimate of compositional plus cell-intrinsic together, composition accounts for ≈ (0.041 − 0.011) / 0.041 ≈ 73% of the bulk Δπ and cell-intrinsic drift for ≈ 27%. This "two-thirds / one-third" partition is also consistent, to within its uncertainty, with the direct single-cell estimate from the 10x cross-balanced analysis above (Δπ ≈ −0.07 per cell type, about a third of the bulk Δπ_residual magnitude). We emphasize that the τ-based partition rests on the assumption that tissue-specific genes are insensitive to non-parenchymal cell-type proportions, which is approximately but not exactly true; the "two-thirds / one-third" numbers should be read as an estimate with ~15-percentage-point uncertainty, not a precise decomposition.
...
*Design note.* The 15-tissue analysis does not use the matched-donor panel from Section 3.1 (263 donors with samples in all six tissues). Instead, each tissue contributes the full set of GTEx donors available for that tissue with ≥ 30 in each age bin. This expands the tissue count from 6 to 15 but introduces a donor-composition caveat: the underlying donor population differs across tissues, so tissue-to-tissue comparisons of absolute Δvar include whatever donor-composition bias exists between the tissue-specific cohorts. We mitigate this with bootstrap CIs over genes (within-tissue) and by interpreting rate *rank ordering* rather than absolute magnitudes. Repeating the analysis on the matched-donor subset reduces tissue count to 6 but the same rank ordering emerges: blood > muscle > thyroid > skin > adipose > artery (Fig. 3, inset).
...
- **Post-mitotic tissues are heterogeneous.** Left-ventricular myocardium shows the *largest* Δvar in the panel (+0.121 [+0.117, +0.124]), atrial appendage is high (+0.063), but aorta, tibial artery, and tibial nerve are near zero or slightly negative. The heart result is a central unexpected finding of this study and deserves explicit treatment.

**Heart as a priority tissue.** Cardiomyocytes renew at ≈ 1% per year in young adults and drop to ≈ 0.45% in old age (Bergmann et al. 2009), so the cardiomyocyte population itself is essentially stationary over the four-decade window we observe. Yet the LV myocardium bulk transcriptome accumulates variance ≈ 50% *faster* than whole blood, the classical high-turnover aging tissue. Three independent mechanisms can generate within-donor transcriptomic heterogeneity without requiring cardiomyocyte division: age-related macrophage and lymphocyte infiltration into myocardium (an increase in non-cardiomyocyte cell types that dilute and distort the bulk signal; Molawi et al. 2014; Ramos et al. 2017); fibroblast activation and progressive diffuse interstitial fibrosis, a near-universal finding in aged hearts (Biernacka & Frangogiannis 2011; Horn & Trafford 2016); and shifts in cardiomyocyte sub-populations, including polyploid and senescent states (Walaszczyk et al. 2019; Anderson et al. 2019). Our bulk variance-decomposition framework cannot attribute the +0.121 between these three sources. What it *does* establish is that the magnitude is larger than any other tissue we tested, and that the canonical blood-centric story of aging-driven transcriptomic noise — while real — is not the whole picture. In particular, heart failure risk rises sharply with age, and a tissue-composition-free readout of cardiac aging that tracks infiltration, fibrosis, and residual cardiomyocyte drift would be a natural target for single-cell follow-up (Litviňuková et al. 2020; Koenig et al. 2022). We therefore propose LV myocardium as a second priority tissue for mechanistic dissection, alongside the well-studied hematopoietic compartment.
...
Species with longer lifespan accumulate transcriptomic noise more slowly by a factor that tracks lifespan — a quantitative echo of earlier scaling laws linking life-history variables (metabolism, heartbeat, cell turnover) to maximum lifespan. One parsimonious reading is that cumulative noise at end of life may be roughly similar across species, with the per-year rate compressed to match lifespan; this reading is consistent with other life-history scaling laws but is not demonstrated mechanistically by our data. An equally plausible alternative is that the scaling we observe reflects confounded covariance of multiple maintenance systems with lifespan rather than a single tuned-maintenance quantity. The cross-species result should therefore be read as a robust empirical scaling relationship — rate inversely proportional to lifespan across four mammals — rather than as evidence for a specific maintenance-capacity mechanism.
...
This result connects naturally with DNA-methylation–clock literature (Horvath 2013; Hannum et al. 2013) without duplicating it. Methylation clocks predict chronological age using tissue-invariant CpG panels — the universal timekeeper. Our transcriptomic residual variance exposes the *tissue-specific consequences* of that timekeeper: blood, heart, and esophagus accumulate noise at rates that differ by an order of magnitude, and neither clock nor framework alone captures the full picture.

The predictive claim that follows is concrete. A methylation clock, applied to a single bulk sample, returns one number — a chronological age estimate. Our framework, applied to the same donor across multiple tissues, returns a tissue-resolved reserve profile. Two donors with identical Horvath ages can have very different profiles: one with near-baseline Δvar in hematopoietic tissue but large Δvar in myocardium, another with the inverse. If this profile reflects functional decline in the respective tissues — a hypothesis that this paper does not prove but makes testable — then the two donors would be predicted to be at different clinical risk for hematological and cardiac events despite being epigenetically the same age. **The joint application of (i) a methylation clock for chronological age estimation and (ii) a transcriptomic variance profile for tissue-specific reserve assessment is therefore expected to stratify individuals of the same biological age into functionally distinct sub-groups.** Testing this prediction requires cohorts with paired multi-tissue transcriptomic and methylation data plus longitudinal clinical outcomes. The UK Biobank, the Baltimore Longitudinal Study of Aging, and the eMERGE network are candidates where this joint analysis is, in principle, executable.
...
CR does not restore aged V_tissue toward young levels; it reduces aged V_residual. Mechanistically, this distinguishes CR from interventions that would have to restore the structural component — for example, Yamanaka-factor–based partial epigenetic reprogramming, which targets chromatin state and should be expected to act on V_tissue. Our framework provides a way to distinguish these mechanisms in transcriptomic readouts, and makes three concrete, individually testable predictions that can be evaluated on existing public datasets without any new experiments:

- **Prediction A (CR-mimetics are noise filters).** Rapamycin and metformin, applied to aged mice long enough to produce a transcriptomic effect, should reduce V_residual in tissues where they act, with little or no change in V_tissue. Data for testing: ITP rapamycin rapalogs, Harrison 2009 and follow-ups (mouse heart, liver, muscle, kidney bulk); Barzilai et al. TAME-preceding metformin cohorts.
- **Prediction B (senolytics act on composition, not within-cell noise).** Dasatinib-plus-quercetin and navitoclax, which remove p16-high senescent cells, should reduce the *compositional* component of V_residual (by removing the cells that contribute the outlier profiles), with smaller effect on within-cell-type variance in the remaining populations. Data for testing: Xu et al. 2018 single-cell and bulk data on D+Q; Baker et al. 2016 p16-INK-ATTAC transgenic clearance transcriptomes. The prediction differentiates senolytics from CR: both reduce aged π_residual but via different variance subcomponents.
- **Prediction C (partial reprogramming is a structure restorer).** Transient OSK/OSKM expression (Ocampo et al. 2016; Browder et al. 2022) should **increase** V_tissue toward young values while having variable effect on V_residual. This is the qualitatively opposite signature from CR and CR-mimetics. Data for testing: Browder 2022 wild-type mouse bulk RNA-seq during and after OSK expression; Chondronasiou et al. 2022 organ-level reprogramming data.

If the framework is correct, Prediction A should yield small/absent V_tissue changes and clear V_residual reductions; Prediction B should yield composition-dominated V_residual reductions; Prediction C should yield V_tissue increases. If any of these three fails, the noise-filter vs structure-restorer dichotomy we propose is wrong, and the framework needs revision. We plan to run these three analyses as a separate follow-up; the Datasets and methods described above are all that is needed.

This mechanistic partition is not obvious from bulk transcriptomic measures other than variance decomposition, and it is the main practical consequence of the framework for intervention biology.
